...
const snapshot = v8Profiler.takeSnapshot("test1");

cp.execSync(
  "rm -rf *.heapsnapshot && rm -rf *.cpuprofile && rm -rf *.heapprofile",
  { cwd: path.join(__dirname) }
);

console.log(
  `================ <node-${version}> test files (before) ================`
);
// const data = cp.execSync('ls -lah', { cwd: path.join(__dirname+'/src.js') })
...
setTimeout(() => {
  ...
  fs.writeFileSync(
    path.join(__dirname, `cpu-node_${version}.cpuprofile`),
...
snapshot.export(function (error, result) {
  ...
  fs.writeFileSync(
    path.join(__dirname, `snapshot-node_${version}-total.heapsnapshot`),
    result
  );
  snapshot.delete();
  count++;
...
totalEvents++;
const snapshot2 = v8Profiler.takeSnapshot("test2");
snapshot2
  .export()
  .pipe(
    fs.createWriteStream(
      path.join(__dirname, `snapshot-node_${version}-stream.heapsnapshot`)
    )
  )
  .on("finish", () => {
    console.log("snapshot stream done...\n");
    snapshot2.delete.bind(snapshot2);
    count++;
  });
// transform.pipe(
//   fs.createWriteStream(
...
setTimeout(() => {
  ...
  require("fs").writeFileSync(
    path.join(__dirname, `heap-node_${version}.heapprofile`),
